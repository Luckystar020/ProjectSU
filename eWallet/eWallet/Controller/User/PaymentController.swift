...
class PaymentController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if metadataObj.type == AVMetadataObject.ObjectType.qr{
            ...
            if metadataObj.stringValue != nil{
                self.WalletID_Store = metadataObj.stringValue!
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        //        if let UserID = Firestore.firestore().collection("Wallet").document(WID).value(forKeyPath: "UserID"){
        //        let UserType = Firestore.firestore().collection("Users").document(UserID as! String).value(forKeyPath: "Usertype") as! Int
        //        if UserType == 2 {
        ...
        //            }
        //        } else if UserType == 1{
        //            let alert = UIAlertController(title: "Not Found ID", message: "This QRCode not support.", preferredStyle: UIAlertControllerStyle.alert)
